...
const Register = () => {
  ...
  const handleRegisterSubmit = (event) => {
    ...
    if (name.length < 5) {
      setError({ ...error, name: "Name Must be at list 6 character" });
      return;
    }
  ...
  return (
    <div className="flex justify-center items-center min-h-screen">
      <div className="card bg-base-100 w-full max-w-xl shrink-0 rounded-none p-10">
        <h2 className="text-center text-2xl py-2 mt-4 font-semibold">
        ...
        <form onSubmit={handleRegisterSubmit} className="card-body">
          ...
          {error.name && (
            <label className="text-xs text-red-500 py-3">{error.name}</label>
          )}
